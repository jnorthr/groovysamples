...
/*
        def cl = new javax.swing.JSpinner.DefaultEditor()
	{
...
*/
	...
	//SpinnerNumberModel
	sModel = new MyModel(12, 6, 72, 1)

//------------
// action logic follows:
def dospinner = swing.action(
  name:'Spinner Value',
  closure:this.&doSpinner)
...
def getFont(int sz)
{
	try
	{
		return new Font("MONOSPACED", Font.PLAIN, sz);
	} 
	catch(Exception x)
	{
...
blockCaret.setBlinkRate(200);   //textArea.getCaret().getBlinkRate());
textArea.setCaret(blockCaret);

textArea.setForeground(Color.GREEN)
textArea.setBackground(Color.BLACK)
font = textArea.getFont();
println "---> the textArea font looks like this:"+font.toString();
...
println "---> and now the textArea font looks like this:"+font.toString();
...
def boxtwo = swing.hbox {
//widget(rowcol)
widget(textArea)
}

def tallbox = swing.vbox{
container(boxone)
container(boxtwo)
} // end of tallbox
      ...
      *   /
      itemStateChanged( ItemEvent e )
      {
		flavour.addItemListener(new ItemListener()
		{
	    	...
	    	{
                println(e.getItem() + " ItemEvent=" + e.getStateChange() );
            	...
            	println( selectedIndex + " " + choice);   // + " " + e.toString() );
            }
		}
      } // end of itemState
}
